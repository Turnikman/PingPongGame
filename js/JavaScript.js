...
function botMove() {
    ...
    if (10 < y && y < game.height - bot.height - 10) {
        bot.y = y;
    }
}
// движение игрока
...
function draw() {
    ...
    context.fillText(bot.scores, 100, 0);
    context.fillText(player.scores, game.width - 100, 0);
    bot.draw(); // левая ракетка
    ...
    if (!start) {
        ...
        //context.textBaseline = 'top';
        //context.fillStyle = '#000';
        //context.fillText("Press on Start", game.width / 2, game.height / 2 - 10);
...
function update() {
    ...
    if (bot.scores === 30 || player.scores === 30) {
        if (bot.scores === 30) { // победа bot
            game.lose++;
            start = false;
            ball.x = game.width - player.width - 1.5 * ball.width - 10;
            ball.y = game.height / 2 - ball.width / 2;
            bot.y = game.height / 2 - bot.height / 2;
            player.y = game.height / 2 - bot.height / 2;
        } else { // победа игрока
            game.win++;
            start = false;
    ...
    if ((collision(bot, ball) && ball.vX < 0) || (collision(player, ball) && ball.vX > 0)) {
        // приращение скорости шарика
        if (ball.vX < 19 && -19 < ball.vX) {
            if (ball.vX < 0) {
                ball.vX--;
            } else {
                ball.vX++;
...
function init() {
    ...
    game = new rect("#000", 0, 0, 520, 390);
    game.total = 0;
    game.win = 0;
    game.lose = 0;
    // Ракетки-игроки

    bot = new rect("#87cefa", 4, game.height / 2 - 50, 10, 100);
    player = new rect("#87cefa", game.width - 14, game.height / 2 - 50, 10, 100);
    // количество очков
    //var rrr=context.fillText(bot.scores, 100, 0);
    // var ttt =context.fillText(player.scores, game.width - 100, 0);
    bot.scores = 0;
    player.scores = 0;

    //var textBox1 = document.getElementById("Text1").value = game.lose;
    //var textBox2 = document.getElementById("Text2").value = player.scores;

    // наш квадратный игровой "шарик"
    ball = new rect("#87cefa", 20, game.height / 2 - 10, 10, 10);
    // скорость шарика
